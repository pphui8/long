...
func (s *TokenStore) RevokeToken(ctx context.Context, jti string) error {
	...
	log.Debug("DAO: Revoking token", zap.String("jti", jti))
	err := s.client.Set(ctx, "revoked:"+jti, "1", RefreshTokenTTL).Err()
	if err != nil {
		log.Error("DAO Error: Failed to revoke token", zap.String("jti", jti), zap.Error(err))
	}
	return err
}
...
func (s *TokenStore) IsRevoked(ctx context.Context, jti string) bool {
	...
	if err != nil {
		log.Error("DAO Error: Failed to check if token is revoked", zap.String("jti", jti), zap.Error(err))
		return false
	}
	return val > 0
}
...
func (s *TokenStore) RegisterToken(ctx context.Context, username string, jti string) error {
	log := logger.WithContext(s.log, ctx)
	log.Debug("DAO: Registering token", zap.String("username", username), zap.String("jti", jti))
	err := s.client.Set(ctx, "active:"+username, jti, RefreshTokenTTL).Err()
	if err != nil {
		log.Error("DAO Error: Failed to register token", zap.String("username", username), zap.Error(err))
	}
	return err
}
...
func (s *TokenStore) InvalidateUserSessions(ctx context.Context, username string) error {
	log := logger.WithContext(s.log, ctx)
	log.Debug("DAO: Invalidating user sessions", zap.String("username", username))
	err := s.client.Del(ctx, "active:"+username).Err()
	if err != nil {
		log.Error("DAO Error: Failed to invalidate user sessions", zap.String("username", username), zap.Error(err))
	}
	return err
}
...
func (s *TokenStore) GetActiveToken(ctx context.Context, username string) string {
	...
	if err != nil && err != redis.Nil {
		log.Error("DAO Error: Failed to get active token", zap.String("username", username), zap.Error(err))
		return ""
	}
	return val
}
